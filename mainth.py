...
def worker(i, topic, sub_title, text, results):
    ...
    for keyword in keywords:
        ...
        if contexts:
            middle_index = len(contexts) // 2
            context = contexts[middle_index]
    ...
    with lock:
        results[i] = paragraph  # Thread-safe write into the result list

def write_paper(points, topic, text):
    outline = get_outline(points, topic)
    sub_titles = split_outline(outline)
    
    results = [None] * len(sub_titles)
    
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        for i, sub_title in enumerate(sub_titles):
            executor.submit(worker, i, topic, sub_title, text, results)

    executor.shutdown()  # Ensure all threads have finished
    
    results = [result for result in results if result is not None]  # Filter out None values
